...
class CustomWebGLCubeRenderTarget extends WebGLCubeRenderTarget {
  ...
  setup(renderer, texture) {
    ...
    let cloneUniforms = require("three/src/renderers/shaders/UniformsUtils.js")
      .cloneUniforms;
...
export class ShaderCubeChrome {
  constructor({ renderer, res = 128, color = new Color("#ffffff") }) {
        ...
        #include <common>
          ...
          void main (void) {
            ...
            gl_FragColor = vec4(vec3(
              1.0 - pattern(uv * 5.0123 + -0.37 * cos(time * 0.05)),
              1.0 - pattern(uv * 5.0123 +  0.0 * cos(time * 0.05)),
              1.0 - pattern(uv * 5.0123 +  0.37 * cos(time * 0.05))
            ) * diffuse, 1.0);
    ...
    this.compute = ({ time }) => {
      uniforms.time.value = time || window.performance.now() * 0.001;
